...
export class ReactionsComponent {
  ...
  async addEmoji(event) {
    ...
    let indexOfCurrentMessage = this.fireService.messages.indexOf(
      this.currentMessage
  ...
  checkForEmoji(indexOfEmoji, newIndexOfEmoji) {
    if (indexOfEmoji === -1) {
      ...
      if (
        this.currentMessage.reactions[indexOfEmoji].userId !==
        this.userService.user.userId
      ) {
  ...
  ifEmojiIsNotOnMessage(newIndexOfEmoji) {
    ...
    newIndexOfEmoji = this.currentMessage.reactions.findIndex(
      (reaction) => reaction.id === this.emoji.id
    );
  }
  ...
  checkForUsersIdForEmoji(indexOfEmoji) {
    return this.currentMessage.reactions[indexOfEmoji].userIDs.findIndex(
      (userID) => userID === this.userService.user.userId
    );
  }
}
